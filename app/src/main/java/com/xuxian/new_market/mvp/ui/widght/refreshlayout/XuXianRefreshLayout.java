package com.xuxian.new_market.mvp.ui.widght.refreshlayout;
...
//                        }
//                    }
                    if (Math.abs(event.getRawX() - this.mInterceptTouchDownX) < ((float) Math.abs(interceptTouchMoveDistanceY))
                            && this.mRefreshHeaderView != null
                            && ((interceptTouchMoveDistanceY > 0 && shouldHandleRefresh()) || ((interceptTouchMoveDistanceY < 0 && shouldHandleLoadingMore()) || (interceptTouchMoveDistanceY < 0 && !isWholeHeaderViewCompleteInvisible())))) {
                        event.setAction(MotionEvent.ACTION_CANCEL);
                        super.onInterceptTouchEvent(event);
                        return true;
                    }
