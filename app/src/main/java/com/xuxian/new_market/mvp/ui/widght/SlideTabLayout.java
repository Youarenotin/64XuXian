...
//            invalidateTabStripBeta();
    ...
    private void invalidateTabStrip() {
        mTabStrip.removeAllViews();
        this.mode = 1;
        this.setBackgroundResource(R.color.light_yellow);
        PagerAdapter adapter = mViewPager.getAdapter();
        if (adapter == null) {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            textView.setTypeface(Typeface.DEFAULT);//默认粗细
    ...
    public class InternalOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            if (mode == 1) {
                ...
                mTabStrip.onPageScrolled(position, positionOffset, (int) (positionOffset * view.getWidth()));
            } else if (mode == 2) {
                View view = mTabStrip.getChildAt(position);
                scrollToTab(position, (int) (positionOffset * view.getWidth()));
...
//                scrollToTab(position, 0);
                Animation animation = AnimationUtils.loadAnimation(mContext, R.anim.anim_tablayout);
                for (int i = 0; i < mTabStrip.getChildCount(); i++) {
                    TextView curTextView = (TextView)(mTabStrip.getChildAt(i));
                    curTextView.setTypeface(Typeface.DEFAULT);
