...
public class Category_one_fragment extends BaseFragment {

    private GridView gridview;

    private  static Category_one_fragment instance;
    private NavGridViewAdapter adapter;
    private ActivityStateView emptyView;

    public static   Category_one_fragment getInstance( ){
        if (instance == null) {
            instance=new Category_one_fragment();
        }
        return instance;
    }

    private  Category_one_fragment() {
...
//        gridview.setOnClickListener(new ItemOnClickListener());
    ...
    private void requestData() {
        Call<CategoryInfo> call = RetrofitManager.getInstance().getInfos("1", "2016083101", System.currentTimeMillis() + "");
        call.enqueue(new Callback<CategoryInfo>() {
            ...
            public void onResponse(Call<CategoryInfo> call, Response<CategoryInfo> response) {
               ...
               else if (response.isSuccessful() && response.body().getStatus().getCode().equals("0") && response.body().getData().get(0).getName().equals("水果")){
